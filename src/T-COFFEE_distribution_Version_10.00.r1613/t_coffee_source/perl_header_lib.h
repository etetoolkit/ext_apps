...
UG_LOCK\"};\nour $debug_generic_method=$ENV{\"DEBU\
...
query=$ARGV[0];\n    $hitf=$ARGV[1];\n    %s=read_\
fasta_seq($query);\n    @sl=keys(%s);\n    &blast_\
xml2profile (\"xx\", $s{$sl[0]}{seq},$maxid,$minid\
,$mincov, $hitf);\n    myexit ($EXIT_FAILURE);\n  \
}\n\nforeach $v(@ARGV){$cl.=\"$v \";}\n$COMMAND=$c\
l;\n($mode)=&my_get_opt ( $cl, \"-mode=\",1,0);\n\\
n($A)=(&my_get_opt ( $cl, \"-name1=\",0,0));\n($B)\
=(&my_get_opt ( $cl, \"-name2=\",0,0));\n($TMPDIR)\
=(&my_get_opt ( $cl, \"-tmpdir=\",0,0));\n($CACHE)\
=(&my_get_opt ( $cl, \"-cache=\",0,0));\n($SERVER)\
=((&my_get_opt ( $cl, \"-server=\",0,0)));\n($EMAI\
L)=((&my_get_opt ( $cl, \"-email=\",0,0)));\n\nif \
(!$A){$A=\"A\";}\nif (!$B){$B=\"B\";}\n\n\nif (!$T\
MPDIR)\n  {\n    $HOME=$ENV{HOME};\n    if ($ENV{T\
MP_4_TCOFFEE}){$TMPDIR=$ENV{TMP_4_TCOFFEE};}\n    \
else{$TMPDIR=\"$HOME/.t_coffee/tmp/\";}\n  }\nif (\
 ! -d $TMPDIR)\n  {\n    mkdir $TMPDIR;\n  }\nif (\
 ! -d $TMPDIR)\n  {\n    print \"ERROR: Could not \
create temporary dir: $TMPDIR\\n\";\n    myexit ($\
EXIT_FAILURE);\n  }\n\n$EMAIL=~s/XEMAILX/\\@/g;\ni\
f (!$EMAIL)\n  {\n    if ($ENV{EMAIL_4_TCOFFEE}){$\
EMAIL=$ENV{EMAIL_4_TCOFFEE};}\n    elsif ($ENV{EMA\
IL}){$EMAIL=$ENV{EMAIL};}\n    else {$EMAIL=$REF_E\
MAIL;}\n  }\n\n($maxid,$minid,$mincov)=(&my_get_op\
t ( $cl, \"-maxid=\",0,0, \"-minid=\",0,0,\"-minco\
v=\",0,0));\nif (!$cl=~/\\-maxid\\=/){$maxid=95;}\\
nif (!$cl=~/\\-minid\\=/){$minid=35;}\nif (!$cl=~/\
\\-mincov\\=/){$mincov=80;}\n\n\n\n\nif ($mode eq \
\"seq_msa\")\n  {\n    &seq2msa($mode,&my_get_opt \
( $cl, \"-infile=\",1,1, \"-method=\",1,2, \"-para\
m=\",0,0,\"-outfile=\",1,0, \"-database=\",0,0));\\
n  }\nelsif ( $mode eq \"tblastx_msa\")\n  {\n    \
&seq2tblastx_lib ($mode,&my_get_opt ( $cl, \"-infi\
le=\",1,1, \"-outfile=\",1,0));\n  }\nelsif ( $mod\
e eq \"tblastpx_msa\")\n  {\n    &seq2tblastpx_lib\
 ($mode,&my_get_opt ( $cl, \"-infile=\",1,1, \"-ou\
tfile=\",1,0));\n  }\nelsif ( $mode eq \"thread_pa\
ir\")\n  {\n    &seq2thread_pair($mode,&my_get_opt\
 ( $cl, \"-infile=\",1,1, \"-pdbfile1=\",1,1, \"-m\
ethod=\",1,2,\"-param=\",0,0, \"-outfile=\",1,0, )\
);\n  }\nelsif ( $mode eq \"pdbid_pair\")\n  {\n  \
  &seq2pdbid_pair($mode,&my_get_opt ( $cl, \"-pdbf\
ile1=\",1,0, \"-pdbfile2=\",1,0, \"-method=\",1,2,\
\"-param=\",0,0, \"-outfile=\",1,0, ));\n  }\nelsi\
f ( $mode eq \"pdb_pair\")\n  {\n    &seq2pdb_pair\
($mode,&my_get_opt ( $cl, \"-pdbfile1=\",1,1, \"-p\
dbfile2=\",1,1, \"-method=\",1,2,\"-param=\",0,0, \
\"-outfile=\",1,0, ));\n  }\nelsif ( $mode eq \"pr\
ofile_pair\")\n  {\n     &seq2profile_pair($mode,&\
my_get_opt ( $cl, \"-profile1=\",1,1, \"-profile2=\
\",1,1, \"-method=\",1,2,\"-param=\",0,0, \"-outfi\
le=\",1,0 ));\n  }\nelsif ($mode eq \"pdb_template\
_test\")\n  {\n    &blast2pdb_template_test ($mode\
,&my_get_opt ( $cl, \"-infile=\",1,1));\n\n  }\nel\
sif ($mode eq \"psi_template_test\")\n  {\n    &ps\
iblast2profile_template_test ($mode,&my_get_opt ( \
$cl, \"-seq=\",1,1,\"-blast=\",1,1));\n\n  }\n\nel\
sif ( $mode eq \"pdb_template\")\n  {\n    &blast2\
pdb_template ($mode,&my_get_opt ( $cl, \"-infile=\\
",1,1, \"-database=\",1,0, \"-method=\",1,0, \"-ou\
tfile=\",1,0,\"-pdb_type=\",1,0));\n  }\n\nelsif (\
 $mode eq \"profile_template\")\n  {\n\n    &psibl\
ast2profile_template ($mode,&my_get_opt ( $cl, \"-\
...
")\n  {\n    &seq2RNA_template ($mode,&my_get_opt \
( $cl, \"-infile=\",1,1, \"-outfile=\",1,0));\n  }\
\nelsif ( $mode eq \"tm_template\")\n  {\n    &seq\
2tm_template ($mode, \"\", &my_get_opt ( $cl, \"-i\
nfile=\",1,1,\"-arch=\",1,1,\"-psv=\",1,1, \"-outf\
ile=\",1,0,));\n  }\nelsif ( $mode eq \"psitm_temp\
late\")\n  {\n    &seq2tm_template ($mode,&my_get_\
opt ( $cl, \"-database=\",1,0, \"-infile=\",1,1, \\
"-arch=\",1,1,\"-psv=\",1,1, \"-outfile=\",1,0,));\
\n  }\nelsif ( $mode eq \"ssp_template\")\n  {\n  \
  &seq2ssp_template ($mode,&my_get_opt ( $cl, \"-i\
nfile=\",1,1,\"-seq=\",1,1,\"-obs=\",1,1, \"-outfi\
le=\",1,0));\n  }\nelsif ( $mode eq \"psissp_templ\
ate\")\n  {\n    &seq2ssp_template ($mode,&my_get_\
opt ( $cl, \"-infile=\",1,1,\"-seq=\",1,1,\"-obs=\\
",1,1, \"-outfile=\",1,0));\n  }\n\nelsif ( $mode \
eq \"rna_pair\")\n{\n    &seq2rna_pair($mode,&my_g\
et_opt ( $cl, \"-pdbfile1=\",1,1, \"-pdbfile2=\",1\
,1, \"-method=\",1,2,\"-param=\",0,0, \"-outfile=\\
",1,0, ));\n}\nelsif ( $mode eq \"calc_rna_templat\
e\")\n{\n    &calc_rna_template($mode,&my_get_opt \
( $cl, \"-infile=\",1,1,\"-pdbfile=\",1,1, \"-outf\
ile=\",1,0));\n}\nelse\n  {\n    myexit(flush_erro\
r( \"$mode is an unknown mode of tc_generic_method\
.pl\"));\n  }\nmyexit ($EXIT_SUCCESS);\n\n\nsub se\
...
\",$outfile, @profiles);\n}\n\nsub seq2tm_template\
\n  {\n  my ($mode, $db, $infile,$arch,$psv,$outfi\
le)=@_;\n  my %s, %h;\n  my $result;\n  my (@profi\
...
ode eq \"tm_template\")\n	{\n	  &safe_system (\"t_\
coffee -other_pg fasta_seq2hmmtop_fasta.pl -in=seq\
file -out=$lib_name -arch=$arch -psv=$psv\");\n	}\\
n      elsif ( $mode eq \"psitm_template\")\n	{\n	\
  &seq2msa_tm_prediction ($s{$seq}{name},$s{$seq}{\
seq}, $db, \"seqfile\", $lib_name,$arch, $psv);\n	\
}\n      if ( !-e $lib_name)\n	{\n	  myexit(flush_\
error(\"RNAplfold failed to compute the secondary \
...
$s{$seq}{name}\\n$s{$seq}{seq}\\n\");\n      close\
 (F);\n      $lib_name=\"$s{$seq}{name}.rfold\";\n\
      $lib_name=&clean_file_name ($lib_name);\n   \
   &safe_system (\"t_coffee -other_pg RNAplfold2tc\
lib.pl -in=seqfile -out=$lib_name\");\n\n      if \
...
2profile_template_test\n  {\n  my ($mode, $seq,$bl\
ast)=@_;\n  my %s, %h, ;\n  my ($result,$psiblast_\
output,$profile_name,@profiles);\n  my $trim=0;\n \
 my $maxid=100;\n  my $minid=0;\n  my $mincov=0;\n\
...
}, $s{$seq}{seq},$maxid, $minid,$mincov,$psiblast_\
...
m);\n	  print stdout \"\\tProcess: >$s{$seq}{name}\
 _R_ $profile_name [$profile{n} Seq.] [$SERVER/bla\
st/$db][$CACHE_STATUS]\\n\";\n	  print R \">$s{$se\
q}{name} _R_ $profile_name\\n\";\n	}\n    }\n  clo\
se (R);\n\n  die;\n}\nsub psiblast2profile_templat\
e\n  {\n  my ($mode, $infile, $db, $method, $outfi\
le)=@_;\n  my %s, %h, ;\n  my ($result,$psiblast_o\
utput,$profile_name,@profiles);\n  my $trim=0;\n  \
...
file\");\n      print (F \">$A\\n$s{$seq}{seq}\\n\\
");\n      close (F);\n      $psiblast_output=&run\
_blast ($s{$seq}{name},$method, $db, \"seqfile\",\\
"outfile\");\n\nif ( -e $psiblast_output)\n	{\n	  \
%profile=blast_xml2profile($s{$seq}{name}, $s{$seq\
}{seq},$maxid, $minid,$mincov,$psiblast_output);\n\
	  unlink ($psiblast_output);\n\n	  $profile_name=\
\"$s{$seq}{name}.prf\";\n	  $profile_name=&clean_f\
ile_name ($profile_name);\n	  unshift (@profiles, \
$profile_name);\n	  output_profile ($profile_name,\
 \\%profile, $trim);\n	  print stdout \"\\tProcess\
: >$s{$seq}{name} _R_ $profile_name [$profile{n} S\
eq.] [$SERVER/blast/$db][$CACHE_STATUS]\\n\";\n	  \
print R \">$s{$seq}{name} _R_ $profile_name\\n\";\\
n	}\n    }\n  close (R);\n  &set_temporary_dir (\"\
unset\",$mode, $method,\"result.aln\",$outfile, @p\
...
ype cannot be used (expected: $etype)]\\n\";\n	   \
   $c++;\n	    }\n	  else\n	    {\n	      $found=1\
;\n	    }\n	}\n\n      if ($found)\n	{\n	  print s\
tdout \"\\t\\t >$s{$seq}{name} _P_ $pdbid\\n\";\n	\
}\n      else\n	{\n	  print stdout \"\\t\\t >$s{$s\
eq}{name} No Template Selected\\n\";\n	}\n      di\
e;\n    }\nsub blast2pdb_template\n  {\n  my ($mod\
e, $infile, $db, $method, $outfile,$type)=@_;\n  m\
...
me},$method, $db, \"seqfile\",\"outfile\");\n\n   \
   %p=blast_xml2profile($s{$seq}{name}, $s{$seq}{s\
eq},$maxid, $minid,$mincov,$blast_output);\n      \
unlink ($blast_output);\n\n      $c=1;\n      prin\
t stdout \"\\tProcess: >$s{$seq}{name} [$SERVER/bl\
ast/$db][$CACHE_STATUS]\\n\";\n      while (!$foun\
d && $c<$p{n})\n	{\n	  $pdbid=&id2pdbid($p{$c}{ide\
ntifyer});\n	  if ( length ($pdbid)>5){$pdbid=id2p\
dbid($p{$c}{definition});}\n\n	  if ( length ($pdb\
id)>5)\n	    {\n	      myexit(add_error (EXIT_FAIL\
URE,$$,$$,getppid(), \"BLAST_FAILURE::Could Not Pa\
rse PDBID ($p{$c}{identifyer},$p{$c}{definition})\\
"));\n	    }\n\n\n	  if (!&pdb_is_released($pdbid)\
)\n	    {\n	      print stdout \"\\t\\t**$pdbid [P\
DB NOT RELEASED or WITHDRAWN]\\n\";\n	      $c++;\\
n	    }\n	  elsif (!&pdb_has_right_type ($pdbid,$t\
ype))\n	    {\n	      my $ptype=&pdb2type ($pdbid)\
;\n	      my $etype=&type2etype($type);\n\n	      \
print stdout \"\\t\\t**$pdbid [$ptype cannot be us\
ed (expected: $etype)]\\n\";\n	      $c++;\n	    }\
\n	  else\n	    {\n	      $found=1;\n	    }\n	}\n\\
n      if ($found)\n	{\n	  print R \">$s{$seq}{nam\
e} _P_ $pdbid\\n\";\n	  print stdout \"\\t\\t >$s{\
$seq}{name} _P_ $pdbid\\n\";\n	}\n      else\n	{\n\
	  print R \">$s{$seq}{name}\\n\";\n	  print stdou\
t \"\\t\\t >$s{$seq}{name} No Template Selected\\n\
\";\n	}\n    }\n  close (R);\n  &set_temporary_dir\
...
ft;\n    my $type=shift;\n\n    my $f=vtmpnam();\n\
\n    my $value= &safe_system (\"t_coffee -other_p\
g extract_from_pdb -model_type $pdb > $f\");\n    \
my $r=&file2string ($f);\n    chomp($r);\n\n\n    \
if ( $r eq \"NMR\" && $type=~/n/){return 1;}\n    \
elsif ( $r eq \"diffraction\" && $type=~/d/){retur\
n 1;}\n    elsif ( $r eq \"model\" && $type=~/m/){\
return 1;}\n    else {return 0;}\n  }\nsub pdb_is_\
released\n  {\n    my $pdb=shift;\n    my $f=vtmpn\
...
d_fasta_seq ($infile);\n    %qs=&fasta_hash2index_\
hash(%qs1);\n\n\n    #&safe_system (\"formatdb -i \
$db\");\n    if ($blast eq \"blastp\"){&safe_syste\
m  (\"blastall -i $infile -d $db -m7 -p blastp -o \
io\");}\n    elsif ($blast eq \"blastn\"){&safe_sy\
stem  (\"blastn -query $infile -db $db -outfmt 5 -\
word_size 4 -out io\");}\n\n    &set_blast_type (\\
"io\");\n\n\n    my %FB=&xml2tag_list (\"io\", \"I\
teration\");\n    open (F, \">$outfile\");\n    pr\
int F \"! TC_LIB_FORMAT_01\\n\";\n    print F \"$s\
{n}\\n\";\n    for ( my $a=0; $a<$s{n}; $a++)\n   \
   {\n	print F \"$s{$a}{name} $s{$a}{len} $s{$a}{s\
eq}\\n\";\n      }\n\n\n    for ( my $a=0; $a<$FB{\
n}; $a++)\n      {\n	my %p=blast_xml2profile ($qs{\
$a}{name}, $qs{$a}{seq},100, 0, 0, $FB{$a}{body});\
\n	my $query=$p{0}{name};\n	my $i= $s1{$query}{ord\
er}+1;\n	for (my $b=1; $b<$p{n}; $b++)\n	  {\n	   \
...
n    close (F);\n    return $output;\n  }\n\nsub b\
last_msa_old\n  {\n    my ($infile,$outfile)=@_;\n\
    my ($a, %seq);\n    %s1=&read_fasta_seq ($infi\
le);\n    foreach $s (keys (%s1))\n      {\n	$i=$s\
1{$s}{order};\n	$s{$i}{name}=$s;\n	$s{$i}{seq}=$s1\
{$s}{seq};\n	$s{$i}{len}=length( $s{$i}{seq});\n	$\
s{n}++;\n      }\n    &safe_system (\"formatdb -i \
$infile\");\n    &safe_system (\"blastall -i $infi\
le -d $infile -m7 -o io\");\n    &set_blast_type (\
...
et_temporary_dir (\"set\",$infile,\"seq.pep\", $da\
tabase, \"db.pep\");\n    $param.=\" >/dev/null 2>\
...
e -in seq.pep -out result.aln $param`;\n      }\n \
   elsif ( $method eq \"probcons\")\n      {\n	`pr\
obcons seq.pep >result.aln 2>/dev/null`;\n      }\\
n    elsif ( $method eq \"mafft\")\n      {\n	`maf\
ft --quiet --localpair --maxiterate 1000 seq.pep> \
result.aln  2>/dev/null`\n      }\n    elsif ( $me\
thod=~/prank/)\n      {\n	`$method -d=seq.pep -o=r\
esult.aln -quiet 2>/dev/null`;\n	`mv result.aln.1.\
fas result.aln`;\n      }\n    elsif ($method eq \\
"clustalo\")\n      {\n	`clustalo -i seq.pep > res\
ult.aln`;\n      }\n    else\n      {\n	`$method -\
infile=seq.pep -outfile=result.aln`;\n      }\n\n \
...
t -in tmp_result.aln -output fasta_aln >result.aln\
\");\n      }\n    elsif ( $method eq \"t_coffee\"\
)\n      {\n	&safe_system (\"t_coffee -in Pstruc.p\
db Sseq.pep Mslow_pair -outfile result.aln -quiet\\
");\n      }\n    else\n      {\n	&safe_system (\"\
$method -infile=seq.pep -pdbfile1=struc.pdb -outfi\
le=result.aln $param>x 2>x\");\n      }\n    &set_\
temporary_dir (\"unset\",$mode,$method,\"result.al\
n\",$outfile);\n    myexit ($EXIT_SUCCESS);\n  }\n\
sub seq2pdbid_pair\n  {\n    my ($mode, $pdbfile1,\
...
_pg dalilite.pl --pdb1 $id1 --chainid1 $c1 --pdb2 \
$id2 --chainid2 $c2 --email=$EMAIL  >dali_stderr 2\
>dali_stderr\";\n	    $dali=`$command`;\n\n	    op\
en (F, \"dali_stderr\");\n	    while (<F>)\n	     \
 {\n		if ( /JobId: dalilite-(\\S+)/)\n		{\n		  $jo\
bid=$1;\n		}\n	      }\n	    close (F);\n	    unli\
nk (\"dali_stderr\");\n\n	    $output1=\"dalilite-\
$jobid.txt\";\n	    if ( -e $output1)\n	      {\n	\
	unlink ($output1);\n		&url2file (\"http://www.ebi\
.ac.uk/Tools/es/cgi-bin/jobresults.cgi/dalilite/da\
lilite-$jobid/aln.html\", \"output2\");\n\n		if ( \
-e \"output2\")\n		  {\n		    my ($seq1, $seq2);\n\
		    $seq1=$seq2=\"\";\n\n		    open (F, \"output\
2\");\n		    while (<F>)\n		      {\n			$l=$_;\n		\
	if ( $l=~/Query\\s+(\\S+)/)\n			  {\n			    $seq1\
...
tput3=~s/\\./-/g;\n			open (F, \">result.aln\");\n\
			print F \"$output3\";\n			close (F);\n		      }\
\n		  }\n	      }\n	  }\n      }\n    &cache_file(\
\"SET\",\"\",\"$name\",\"$method\",\"dali\",\"resu\
lt.aln\",\"EBI\");\n    &set_temporary_dir (\"unse\
t\",$mode, $method, \"result.aln\",$outfile);\n   \
 myexit ($EXIT_SUCCESS);\n  }\nsub seq2pdb_pair\n \
 {\n    my ($mode, $pdbfile1, $pdbfile2, $method, \
$param, $outfile)=@_;\n\n    &set_temporary_dir (\\
"set\",$pdbfile1,\"pdb1.pdb\",$pdbfile2,\"pdb2.pdb\
\");\n    if ($method eq \"t_coffee\")\n      {\n	\
&safe_system (\"t_coffee -in Ppdb1.pdb Ppdb2.pdb -\
quiet -outfile=result.aln\");\n      }\n    elsif \
( $method eq \"DaliLite\")\n      {\n	if ( &safe_s\
ystem (\"DaliLite -pairwise pdb1.pdb pdb2.pdb >tmp\
1\")==$EXIT_SUCCESS)\n	  {\n	     my ($seq1, $seq2\
);\n	     $seq1=$seq2=\"\";\n\n	     open (F, \"tm\
p1\");\n	     while (<F>)\n	       {\n		 $l=$_;\n	\
	 if ( $l=~/Query\\s+(\\S+)/)\n		   {\n		     $seq\
...
ric_method.pl]\\n\";\n	  }\n      }\n    elsif ( $\
method eq \"TMalign\")\n      {\n	if ( &safe_syste\
m (\"TMalign pdb1.pdb pdb2.pdb >tmp1\")==$EXIT_SUC\
CESS)\n	  {\n	    `tail -4 tmp1 > tmp2`;\n\n	    o\
pen (F, \"tmp2\");\n	    while (<F>)\n	      {\n		\
unshift(@l, $_);\n	      }\n	    close (F);\n	    \
open (F, \">result.aln\");\n	    $l[3]=~s/[^a-zA-Z\
...
n		`clustalw -profile1=prf1.aln -profile2=prf2.aln\
 -outfile=result.aln`;\n		&set_temporary_dir (\"un\
set\",$mode, $method, \"result.aln\",$outfile);\n	\
}\n	elsif ( $method eq \"clustalo\")\n	{\n		`clust\
alo --p1 $profile1 --p2 $profile2 -o $outfile --fo\
...
turn 0;}\n      }\n    else\n      {\n	$r=`which $\
p 2>/dev/null`;\n	if ($r eq \"\"){$r=0;}\n	else {$\
r=1;}\n\n	if ($r==0 && is_blast_package ($p)){retu\
rn pg_is_installed (\"legacy_blast.pl\");}\n	else \
...
lled on your system\"));\n      }\n    else\n     \
 {\n	return 1;\n      }\n  }\nsub set_temporary_di\
r\n  {\n    my @list=@_;\n    my $dir_mode, $a, $m\
ode, $method;\n\n    $dir_mode=shift (@list);\n\n\\
n    if ( $dir_mode eq \"set\")\n      {\n	$initia\
l_dir=cwd();\n	if ( !$tmp_dir)\n	  {\n	    $rand=r\
and (100000);\n	    $tmp_dir=\"$TMPDIR/tmp4tcoffee\
_profile_pair_dir_$$\\_P_$rand\";\n	  }\n	if ( !-d\
 $tmp_dir)\n	  {\n	    push (@TMPDIR_LIST, $tmp_di\
r);\n	    `mkdir $tmp_dir`;\n	  }\n\n	for ( $a=0; \
$a<=$#list; $a+=2)\n	      {\n		if (-e $list[$a]){\
...
produce $list[1]\" ));\n	    myexit ($EXIT_FAILURE\
    ...
    # `t_coffee -other_pg seq_reformat -in $tmp_di\
r/$list[0] -output fasta_aln -out $tmp_dir/result2\
.aln`;\n	    `cp $tmp_dir/$list[0] $tmp_dir/result\
2.aln`;\n	    if ( $list[1] eq \"stdout\")\n	     \
...
(@argl,$argv);\n\n\n	#$optional:0=>optional\n	#$op\
tional:1=>must be set\n	#$status: 0=>no requiremen\
t\n	#$status: 1=>must be an existing file\n	#$stat\
us: 2=>must be an installed package\n\n\n	if ($opt\
...
exit(flush_error( \"File $argv must exist\"));\n	 \
   myexit ($EXIT_FAILURE);\n	  }\n	elsif ( $status\
==2 && $argv ne \"\" && &check_pg_is_installed ($a\
...
m=~s/#S/ /g;\n    $param=~s/#M/\\-/g;\n    $param=\
~s/#E/\\=/g;\n\n\n\n    $command=\"hhalign -i $h{0\
}{a3m} -t $h{1}{a3m} -tc $outfile.tmp -rank 1 -map\
t 0 $param\";\n    `$command`;\n\n  #  `hhalign -i\
 $h{0}{a3m} -t $h{1}{a3m} -tc $outfile.tmp -rank 1\
 -mapt 0 -gapf 0.8 -gapg 0.8`;\n\n\n    # To run g\
lobal use the following\n\n    open (I, \"$outfile\
.tmp\");\n    open (O, \">$outfile\");\n    $h{0}{\
cons}=s/\\./x/g;\n    $h{1}{cons}=s/\\./x/g;\n\n  \
...
le\n  {\n    my (%h)=@_;\n    my ($aln,$i,$hv, $a,\
 @c, $n);\n\n\n    $i=$h{index};\n    $aln=$h{aln}\
;\n\n    `cp $aln $$.hhh_aln`;\n    $command=\"t_c\
offee -other_pg seq_reformat -in $aln -output hasc\
h\";\n    $hv=`$command`;chomp ($hv);\n\n    $h{a2\
m}=\"$tmp/$hv.tmp4hhpred.a2m\";\n    $h{a3m}=\"$tm\
p/$hv.tmp4hhpred.a3m\";\n    if ( -e $h{a3m}){;}\n\
    else\n      {\n	$x=`which hhconsensus`;\n	`hhc\
onsensus  -M 50 -i $h{aln} -oa2m $h{a2m}`;\n	if (!\
-e $h{a2m})\n	  {\n	    print STDERR \"Program tc_\
generic_method.pl FAILED to run:\\n\\thhconsensus \
 -M 50 -i $h{aln} -oa2m $h{a2m}\";\n	    myexit ($\
EXIT_FAILURE);\n	  }\n\n	`hhconsensus  -M 50 -i $h\
{aln} -oa3m $h{a3m}`;\n	if (!-e $h{a3m})\n	  {\n	 \
   print STDERR \"Program tc_generic_method.pl FAI\
LED to run:\\n\\thhconsensus  -M 50 -i $h{aln} -oa\
3m $h{a3m}\";\n	    myexit ($EXIT_FAILURE);\n	  }\\
n       `buildali.pl $h{a3m} -n 1`;\n      }\n\n\n\
    $h{a2m_seq}=`head -n 2 $h{a2m} | grep -v \">\"\
`;chomp ($h{a2m_seq});\n    $h{a3m_seq}=`head -n 2\
 $h{a3m} | grep -v \">\"`;chomp ($h{a3m_seq});\n  \
  $h{cons}=$h{a2m_seq};\n    $h{seq}=`head -n 2 $h\
{aln} | grep -v \">\"`;chomp ($h{seq});\n\n\n\n   \
 @c=split (//, $h{cons});\n    $h{len}=$#c+1;\n   \
...
FILE, \"+<$in\");\n\n	my $line;\n\n	if ($mode eq \\
"code\")\n	{\n		chomp($line = <INFILE>);\n		my $li\
ne_length = length($line);\n		$new_name=++$RECODE_\
N;\n		$new_name=\">$new_name\";\n		my $new_length \
= length($new_name);\n		$RECODE {$new_name}=$line;\
\n		for ($count = $new_length; $count < $line_leng\
th; $count++)\n		{\n			$new_name .= \" \";\n		}\n	\
	$new_name=\"$new_name\\n\";\n		seek INFILE, 0, 0\\
n			or die \"could not seek: $!\";\n		print INFILE\
 \"$new_name\";\n	}\n	else\n	{\n		my $n_found = 0;\
...
\n	my $f=new FileHandle;\n	my %seq;\n	my $new_name\
;\n\n	if (! -e $in){return;}\n\n	#needed by Clusta\
lOmega to avoid very long names\n	%seq=read_fasta_\
aln ($in);\n\n	open ($f, \">$in\");\n	foreach my $\
s (keys(%seq))\n	{\n		if ($mode eq \"code\")\n		{\\
n			$new_name=++$RECODE_N;\n			$RECODE {$new_name}\
=$seq{$s}{name};\n		}\n		else\n		{\n			$new_name=$\
RECODE{$seq{$s}{name}};\n		}\n		print $f \">$new_n\
ame\\n$seq{$s}{seq}\\n\";\n	}\n	close $f;\n}\n\nsu\
b fasta_hash2index_hash\n  {\n    my %s1=@_;\n    \
my %s;\n    foreach my $s (keys (%s1))\n      {\n	\
my $i=$s1{$s}{order};\n	$s{$i}{name}=$s;\n	$s{$i}{\
seq}=$s1{$s}{seq};\n	$s{$i}{len}=length( $s{$i}{se\
q});\n	$s{n}++;\n      }\n    return %s;\n  }\nsub\
 file_contains\n  {\n    my ($file, $tag, $max)=(@\
_);\n    my ($n);\n    $n=0;\n\n    if ( !-e $file\
 && ($file =~/$tag/)) {return 1;}\n    elsif ( !-e\
...
file=shift;\n      my $line;\n\n\n      if ( !-e $\
file) {return 0;}\n      $line=&file2tail ($file,1\
00);\n\n      if ( $line=~/<\\/EBIApplicationResul\
t/ || $line=~/<\\/NCBI_BlastOutput/ || $line=~/<\\\
/BlastOutput/ ){return 1;}\n      return 0;\n    }\
\nsub file2blast_flavor\n      {\n	my $file=shift;\
\n	if (&file_contains ($file,\"EBIApplicationResul\
t\",100)){return \"EBI\";}\n	elsif (&file_contains\
 ($file,\"NCBI_BlastOutput\",100)){return \"NCBI\"\
;}\n	else {return \"UNKNOWN\";}\n      }\nsub blas\
...
T_TYPE eq \"NCBI\" || &file_contains ($file,\"NCBI\
_BlastOutput\",100)){%p=ncbi_blast_xml2profile(@_)\
;}\n    else\n      {\n	myexit(add_error ( $$,$$,g\
etppid(), \"BLAST_FAILURE::unkown XML\",$CL));\n  \
    }\n    for ($a=0; $a<$p{n}; $a++)\n      {\n	m\
y $name=$p{$a}{name};\n	$p{$name}{seq}=$p{$a}{seq}\
;\n	$p{$name}{index}=$a;\n      }\n    return %p;\\
n  }\nsub ncbi_tblastx_xml2lib_file\n  {\n    my  \
($outlib,$string)=(@_);\n    my ($L,$l, $a,$b,$c,$\
d,$i,$nhits,@identifyerL);\n    my (%ITERATION);\n\
\n    open (F, \">>$outlib\");\n\n    $seq=~s/[^a-\
zA-Z]//g;\n    $L=length ($seq);\n\n    %ITERATION\
=xml2tag_list ($string, \"Iteration\");\n    for (\
$i=0; $i<$ITERATION{n};$i++)\n      {\n	my ($qinde\
x, $qlen, %hit, $string);\n	$string=$ITERATION{$i}\
{body};\n\n	$qindex=xmltag2value($string,\"Iterati\
on_iter-num\");\n	$qlen  =xmltag2value($string,\"I\
teration_query-len\");\n	%hit=&xml2tag_list  ($str\
ing, \"Hit\");\n\n	for ($a=0; $a<$hit{n}; $a++)\n	\
  {\n	    my ($string);\n	    $string=$hit{$a}{bod\
y};\n\n	    $hindex=xmltag2value($string,\"Hit_acc\
ession\")+1;\n	    if ($hindex<=$qindex){next;}\n	\
    else  {print F  \"# $qindex $hindex\\n\";}\n\n\
\n	    $hlen=xmltag2value  ($string,\"Hit_len\");\\
n	    %HSP=&xml2tag_list  ($string, \"Hsp\");\n\n	\
    for ($b=0; $b<$HSP{n}; $b++)\n	      {\n		my (\
$string, $qs,$qe,$qf,$hs,$he,$hf,$s, $d, $e);\n		$\
string=$HSP{$b}{body};\n\n		$qs=xmltag2value  ($st\
ring,\"Hsp_query-from\");\n		$qe=xmltag2value  ($s\
tring,\"Hsp_query-to\");\n		$qf=xmltag2value  ($st\
ring,\"Hsp_query-frame\");\n\n		$hs=xmltag2value  \
($string,\"Hsp_hit-from\");\n		$he=xmltag2value  (\
$string,\"Hsp_hit-to\");\n		$hf=xmltag2value  ($st\
ring,\"Hsp_hit-frame\");\n\n		$s=xmltag2value  ($s\
tring,\"Hsp_identity\");\n		$l=xmltag2value  ($str\
...
i=0; $i<$ITERATION{n};$i++)\n      {\n	my ($qindex\
, $qlen, %hit, $string);\n	$string=$ITERATION{$i}{\
body};\n\n	$qdef=xmltag2value($string,\"Iteration_\
query-def\");\n	%qdes=&tblastpx_name2description($\
qdef,%s);\n	$qlen  =xmltag2value($string,\"Iterati\
...
"Hit\");\n\n	for ($a=0; $a<$hit{n}; $a++)\n	  {\n	\
 ...
 if ($hdes{index}<=$qdes{index}){next;}\n	    else\
  {print F  \"# $qdes{index} $hdes{index}\\n\";}\n\
\n\n	    $hlen=xmltag2value  ($string,\"Hit_len\")\
;\n	    %HSP=&xml2tag_list  ($string, \"Hsp\");\n\\
n	    for ($b=0; $b<$HSP{n}; $b++)\n	      {\n		my\
 ($string, $l,$qs,$qe,$qf,$hs,$he,$hf,$s, $d, $e, \
@s1, @s2);\n		$string=$HSP{$b}{body};\n\n		$qs=xml\
tag2value  ($string,\"Hsp_query-from\");\n		$qe=xm\
ltag2value  ($string,\"Hsp_query-to\");\n		$qf=$qd\
es{frame};\n		$qseq=xmltag2value  ($string,\"Hsp_q\
seq\");\n\n		$hs=xmltag2value  ($string,\"Hsp_hit-\
from\");\n		$he=xmltag2value  ($string,\"Hsp_hit-t\
o\");\n		$hf=$hdes{frame};\n		$hseq=xmltag2value  \
($string,\"Hsp_hseq\");\n\n		$s=xmltag2value  ($st\
ring,\"Hsp_identity\");\n		$l=xmltag2value  ($stri\
ng,\"Hsp_align-len\");\n		$s=int(($s*100)/$l);\n		\
@s1=tblastpx_hsp2coordinates($qseq,$qs,$qe,%qdes);\
\n		@s2=tblastpx_hsp2coordinates($hseq,$hs,$he,%hd\
es);\n\n\n		for ($f=0; $f<=$#s1; $f++)\n		  {\n		 \
   if ($s1[$f]==-1 || $s2[$f]==-1){next;}\n		    e\
lse\n		      {\n			print F \" $s1[$f] $s2[$f] $s 0\
\\n\";\n		      }\n		  }\n	      }\n	  }\n      }\\
n    print F \"! SEQ_1_TO_N\\n\";\n\n    close (F)\
;\n    return %lib;\n  }\nsub tblastpx_hsp2coordin\
ates\n  {\n    my ($seq, $s, $e, %des)=@_;\n    my\
 @list;\n    my @sa;\n    my @gap=(-1,-1,-1);\n\n \
   $s=$des{start}+3*($s-1);\n\n    if ($des{strand\
...
d}=$at[3];\n    $des{length}=$at[4];\n    $des{ind\
ex}=$s{$at[0]}{order}+1;\n    return %des;\n  }\ns\
ub ncbi_blast_xml2profile\n  {\n    my ($name,$seq\
,$maxid, $minid, $mincov, $string)=(@_);\n    my (\
$L,$l, $a,$b,$c,$d,$nhits,@identifyerL);\n\n\n    \
$seq=~s/[^a-zA-Z]//g;\n    $L=length ($seq);\n\n  \
  #This is causing the NCBI parser to fail when It\
eration_query-def is missing\n    #%query=&xml2tag\
_list ($string, \"Iteration_query-def\");\n    #$n\
ame=$query{0}{body};\n\n    %hit=&xml2tag_list ($s\
tring, \"Hit\");\n\n\n    for ($nhits=0,$a=0; $a<$\
hit{n}; $a++)\n      {\n	my ($ldb,$id, $identity, \
...
l2tag_list ($HSP{$b}{body}, \"Hsp_hit-from\");\n\n\
...
identifyer;\n		$comment[$nhits]=\"$ldb|$identifyer\
 [Eval=$expectation][id=$identity%][start=$Hstart \
end=$Hend]\";\n		$nhits++;\n	      }\n	  }\n      \
}\n\n\n    $profile{n}=0;\n    $profile{$profile{n\
...
M{$b}{open}, \"start\");\n	    $endM=&tag2value ($\
M{$b}{open}, \"end\");\n	    $coverage=(($end-$sta\
rt)*100)/$L;\n\n	   # print \"$id: ID: $identity C\
OV: $coverage [$start $end]\\n\";\n\n	    if ($ide\
...
ile{$n}{identifyer}=$identifyerL[$a];\n	$profile{$\
n}{comment}=$comment[$a];\n\n	for ($b=0; $b<$L; $b\
++)\n	  {\n	    if ($p[$a][$b])\n	      {\n		$prof\
ile{$n}{seq}.=$p[$a][$b];\n	      }\n	    else\n	 \
     {\n		$profile{$n}{seq}.=\"-\";\n	      }\n	  \
...
n %profile;\n  }\nsub output_profile\n  {\n    my \
($outfile,$profileR, $trim)=(@_);\n    my ($a);\n \
   my %profile=%$profileR;\n    my $P= new FileHan\
dle;\n    my $tmp=vtmpnam();\n\n    open ($P, \">$\
tmp\");\n    for ($a=0; $a<$profile{n}; $a++)\n   \
   {\n	print $P \">$profile{$a}{name} $profile{$a}\
{comment}\\n$profile{$a}{seq}\\n\";\n      }\n    \
close ($P);\n\n    if ( $trim)\n      {\n	&safe_sy\
stem (\"t_coffee -other_pg seq_reformat -in $tmp -\
action +trim _aln_%%$trim\\_K1 -output fasta_aln -\
out $outfile\");\n      }\n    else\n      {\n	&sa\
fe_system (\"mv $tmp $outfile\");\n      }\n    re\
turn;\n  }\nsub blast_xml2hit_list\n  {\n    my $s\
tring=(@_[0]);\n    return &xml2tag_list ($string,\
 \"hit\");\n  }\nsub xmltag2value\n  {\n    my ($s\
tring_in, $tag)=@_;\n    my %TAG;\n    %TAG=xml2ta\
g_list ($string_in, $tag);\n    return $TAG{0}{bod\
y};\n  }\n\nsub xml2tag_list\n  {\n    my ($string\
_in,$tag)=@_;\n    my $tag_in, $tag_out;\n    my %\
tag;\n\n    if (-e $string_in)\n      {\n	$string=\
...
r_obs)=(@_);\n    my ($l);\n\n    `gorIV -prd $inf\
ile -seq $gor_seq -obs $gor_obs > gor_tmp`;\n    o\
pen (GR, \">$outfile\");\n    open (OG, \"gor_tmp\\
");\n\n    while (<OG>)\n      {\n\n	$l=$_;\n	if (\
$l=~/\\>/){print GR \"$l\";}\n	elsif ( $l=~/Predic\
ted Sec. Struct./)\n	  {\n	    $l=~s/Predicted Sec\
...
\"GET\",$infile,$name,$method,$db,$outfile,$SERVER\
) && is_valid_blast_xml ($outfile))\n      {return\
 $outfile;}\n    else\n      {\n	$CACHE_STATUS=\"C\
OMPUTE CACHE\";\n	if ( $SERVER eq \"EBI_SOAP\")\n	\
  {\n	    &check_configuration (\"EMAIL\",\"SOAP::\
Light\",\"INTERNET\");\n\n	    $cl_method=$method;\
\n	    if ($cl_method =~/wu/)\n	      {\n		$cl_met\
hod=~s/wu//;\n		if ( $cl_method eq \"psiblast\")\n\
		  {\n		    add_warning($$,$$,\"PSI BLAST cannot \
be used with the wuBLAST Client. Use server=EBI Or\
 server=LOCAL. blastp will be used instead\");\n		\
    $cl_method=\"blastp\";\n		  }\n\n		$command=\"\
t_coffee -other_pg wublast.pl --email $EMAIL $infi\
le -D $db -p $cl_method --outfile $outfile -o xml>\
/dev/null 2>$error_log\";\n		&safe_system ( $comma\
nd);\n		if (-e \"$outfile.xml\") {`mv $outfile.xml\
 $outfile`;}\n	      }\n	    else\n	      {\n		if \
($cl_method eq \"psiblast\"){$cl_method =\"blastp \
-j5\";}\n\n		$command=\"t_coffee -other_pg blastpg\
p.pl --email $EMAIL $infile -d $db --outfile $outf\
ile -p $cl_method --mode PSI-Blast>/dev/null 2>$er\
ror_log\";\n		&safe_system ( $command);\n\n		if (-\
e \"$outfile.xml\") {`mv $outfile.xml $outfile`;}\\
n	      }\n	  }\n	elsif ($SERVER eq \"EBI_REST\" |\
| $SERVER eq \"EBI\")\n	  {\n	    $cl_method=$meth\
od;\n	    &check_configuration(\"EMAIL\",\"XML::Si\
mple\", \"INTERNET\");\n	    if ($db eq \"uniprot\\
"){$db1=\"uniprotkb\";}\n	    else {$db1=$db;}\n\n\
	   \n	    if ($cl_method =~/wu/)\n	      {\n		$cl\
_method=~s/wu//;\n		if ( $cl_method eq \"psiblast\\
...
-p $cl_method --outfile $outfile --align 5 --stype\
 protein $infile>/dev/null 2>error_log\";\n	      \
}\n	    else\n	      {\n		if ( $cl_method =~/psibl\
ast/){$cl_method =\"blastp -j5\";}\n		$command=\"t\
...
\n	      }\n	    &safe_system ( $command,5);\n	   \
 if (-e \"$outfile.out.xml\") {`mv $outfile.out.xm\
l $outfile`;}\n	    elsif (-e \"$outfile.xml.xml\"\
){`mv $outfile.xml.xml $outfile`;}\n	    elsif (-e\
 \"$outfile.out..xml\") {`mv $outfile.out..xml $ou\
tfile`;}\n	    elsif (-e \"$outfile.xml..xml\"){`m\
v $outfile.xml..xml $outfile`;}\n	  }\n	elsif ($SE\
RVER eq \"NCBI\")\n	  {\n	    &check_configuration\
 (\"INTERNET\");\n	    if ($db eq \"uniprot\"){$cl\
_db=\"swissprot\";}\n	    else {$cl_db=$db;}\n\n	 \
...
x$db\";\n	      }\n	    else\n	      {\n		$cl_db=$\
db;\n	      }\n\n        ##\n		## BLAST+ provide d\
ifferent binaries names and CLI options\n		## Use \
the 'legacy_blast.pl' to keep compatibility with o\
ld blast commands\n		##\n		$path=`which legacy_bla\
st.pl 2>/dev/null`;  \n		$path=`dirname $path`; \n\
		chomp($path);\n	    if ($method eq \"blastp\")\n\
...
\n	      }\n	    elsif ($method eq \"blastn\")\n	 \
     {\n		&check_configuration(\"legacy_blast.pl\"\
);\n		$command=\"legacy_blast.pl blastall --path $\
path -p blastn -d $cl_db -i $infile -o $outfile -m\
7 -W6\";\n	      }\n	    &safe_system ($command);\\
n	  }\n	else\n	  {\n\n	    myexit(add_error (EXIT_\
FAILURE,$$,$$,getppid(), \"BLAST_FAILURE::UnknownS\
erver\",$CL));\n	  }\n\n\n	#Check that everything \
went well\n\n	if ( !-e $outfile || !&is_valid_blas\
t_xml($outfile))\n	  {\n\n	    if ( -e $outfile)\n\
	      {\n		add_warning ($$,$$,\"Corrupted Blast O\
utput (Run $run)\");\n		unlink($outfile);\n	      \
}\n	    if ( -e $error_log)\n	      {\n\n		my $err\
or_msg=file2string ($error_log);\n\n		if ( $error_\
msg =~/enter a valid email/)\n		  {\n		    myexit(\
add_error (EXIT_FAILURE,$$,$$,getppid(), \"BLAST_F\
AILURE::Invalid_or_rejected_email::$EMAIL\", \"$co\
mmand\"));\n		  }\n	      }\n	    if ( $run==$BLAS\
T_MAX_NRUNS)\n	      {\n\n		myexit(add_error (EXIT\
_FAILURE,$$,$$,getppid(), \"BLAST_FAILURE::Unknown\
Reason\", \"$command\"));\n	      }\n	    else\n	 \
     {\n		my $out;\n		if ($SERVER eq \"NCBI\") {$S\
ERVER=\"EBI\"; }\n		elsif ($SERVER eq \"EBI\"){$SE\
RVER=\"NCBI\";}\n		add_warning ($$,$$,\"Blast for \
$name failed (Run: $run out of $BLAST_MAX_NRUNS. U\
...
utfile ~/Dropbox/tmp/cedric.out\");\n	#die;\n	retu\
rn $outfile;\n      }\n  }\n\nsub cache_file\n  {\\
n    my ($cache_mode,$infile,$name,$method,$db, $o\
utfile,$server)=(@_);\n    my $cache_file;\n    #P\
rotect names so that they can be turned into legal\
 filenames\n    $name=&clean_file_name ($name);\n\\
n    if ($db=~/\\//)\n      {\n	$db=~/([^\\/]+)$/;\
\n	$db=$1;\n      }\n    $cache_file_sh=\"$name.$m\
ethod.$db.$server.tmp\";\n    $cache_file=\"$CACHE\
/$name.$method.$db.$server.tmp\";\n\n    if ($infi\
le ne \"\")\n      {\n	$cache_file_infile_sh=\"$na\
me.$method.$db.$server.infile.tmp\";\n	$cache_file\
_infile=\"$CACHE/$name.$method.$db.$server.infile.\
...
: $CACHE Does not Exist\";\n	    return 0;\n	  }\n\
	else\n	  {\n	    if ( -e $cache_file && &fasta_fi\
le1_eq_fasta_file2($infile,$cache_file_infile)==1)\
...
CACHE Does not Exist\";\n	    return 0;\n	  }\n	el\
sif (-e $outfile)\n	  {\n	    `cp $outfile $cache_\
file`;\n	    if ($cache_file_infile ne \"\"){ `cp \
$infile $cache_file_infile`;}\n\n	    #functions f\
or updating the cache\n	    #`t_coffee -other_pg c\
lean_cache.pl -file $cache_file_sh -dir $CACHE`;\n\
	    #`t_coffee -other_pg clean_cache.pl -file $ca\
che_file_infile_sh -dir $CACHE`;\n	    return 1;\n\
...
))\n	{\n	  return &safe_system (\"wget $address -O\
$out >/dev/null 2>/dev/null\");\n	}\n    elsif (&p\
g_is_installed (\"curl\"))\n      {\n	return &safe\
_system (\"curl $address -o$out >/dev/null 2>/dev/\
null\");\n      }\n    else\n      {\n	myexit(flus\
_error(\"neither curl nor wget are installed. Imnp\
ossible to fectch remote file\"));\n	exit ($EXIT_F\
AILURE);\n      }\n  }\nsub fasta_file1_eq_fasta_f\
ile2\n  {\n    my ($f1, $f2)=@_;\n    my (%s1, %s2\
);\n    my @names;\n    %s1=read_fasta_seq ($f1);\\
n    %s2=read_fasta_seq ($f2);\n\n    @names=(keys\
 (%s1));\n\n    foreach $n (keys(%s1))\n      {\n	\
...
ate_h{$seq};\n		$lib_name=\"$s{$seq}{name}.rfold\"\
;\n		$lib_name=&clean_file_name ($lib_name);\n		sa\
fe_system (\"secondary_struc.py seqfile $CACHE$pdb\
_chain  $lib_name\");\n		\n		if ( !-e $lib_name)\n\
		{\n		myexit(flush_error(\"secondary_struc.py fai\
led to compute the secondary structure of PDB  $s{\
$seq}{name}\"));\n			myexit ($EXIT_FAILURE);\n		}\\
n		else\n		{\n			print stdout \"\\tProcess: >$s{$s\
eq}{name} _F_ $lib_name\\n\";\n			print R \">$s{$s\
eq}{name} _F_ $lib_name\\n\";\n		}\n		unshift (@pr\
ofiles, $lib_name);\n	}\n	close (R);\n	&set_tempor\
ary_dir (\"unset\",$mode, $method,\"result.aln\",$\
outfile, @profiles);\n}\n\n\n\nsub seq2rna_pair{\n\
	my ($mode, $pdbfile1, $pdbfile2, $method, $param,\
...
str($line,length($line)-3,1);\n	  close TMP;\n\n	 \
 $tmp_file=&vtmpnam();\n	  \n	  safe_system(\"runs\
ara.py $pdbfile1 $chain1 $pdbfile2 $chain2 -s -o $\
tmp_file --limitation 5000 > /dev/null 2> /dev/nul\
l\") == 0 or die \"sara did not work $!\\n\";\n	  \
open(TMP,\"<$tmp_file\") or die \"cannot open the \
sara tmp file:$!\\n\";\n	  open(OUT,\">$outfile\")\
...
p F\");\n    &safe_system (\"blastall -p tblastx -\
i infile -d infile -m 7 -S1>blast.output\");\n\n  \
  ncbi_tblastx_xml2lib_file (\"outfile\", file2str\
ing (\"blast.output\"));\n    &set_temporary_dir (\
...
ormat -in infile -output tblastx_db1 > tblastxdb\"\
);\n    &safe_system (\"formatdb -i tblastxdb -p T\
\");\n    &safe_system (\"blastall -p blastp -i tb\
lastxdb -d tblastxdb -m7 >blast.output\");\n    nc\
bi_tblastpx_xml2lib_file (\"outfile\", file2string\
 (\"blast.output\"), %s);\n    &set_temporary_dir \
(\"unset\",$mode, $method, \"outfile\",$outfile);\\
...
id,$type,$action,$value)=@_;\n	  my $fname;\n	  my\
 $host=hostname;\n\n	  if ($type eq \"LLOCK\"){$fn\
ame=\"$LOCKDIR/.$pid.$host.lock4tcoffee\";}\n	  el\
sif ( $type eq \"LERROR\"){ $fname=\"$LOCKDIR/.$pi\
d.$host.error4tcoffee\";}\n	  elsif ( $type eq \"L\
WARNING\"){ $fname=\"$LOCKDIR/.$pid.$host.warning4\
tcoffee\";}\n\n	  if ($debug_lock)\n	    {\n	     \
 print STDERR \"\\n\\t---lock4tc(tcg): $action => \
...
\n    my $rpid=shift;\n    my $pid =shift;\n    my\
 $command=shift;\n    my $msg=\"$$ -- WARNING: $co\
mmand\\n\";\n    print STDERR \"$msg\";\n    lock4\
tc ($$, \"LWARNING\", \"LSET\", $msg);\n  }\n\nsub\
 signal_cleanup\n  {\n    print dtderr \"\\n**** $\
$ (tcg) was killed\\n\";\n    &cleanup;\n    exit \
($EXIT_FAILURE);\n  }\nsub clean_dir\n  {\n    my \
$dir=@_[0];\n    if ( !-d $dir){return ;}\n    els\
if (!($dir=~/tmp/)){return ;}#safety check 1\n    \
...
d;\n  my $status;\n  my $ppid=getppid();\n  if ($c\
om eq \"\"){return 1;}\n\n  if ( ($com=~/^blast/) \
||($com=~/^formatdb/)){$com=&blast_com2new_blast_c\
...
L\"));\n	    }\n	}\n      return 1;\n    }\n\n$pro\
gram=\"T-COFFEE (Version_10.00.r1613)\";\n\n","*TC\
_METHOD_FORMAT_01\n******************generic_metho\
d.tc_method*************\n*\n*       Incorporating\
...
NV{\"LOCKDIR_4_TCOFFEE\"};\nif (!$LOCKDIR){$LOCKDI\
R=getcwd();}\nour $ERRORDIR=$ENV{\"ERRORDIR_4_TCOF\
...
E=1;\nour $CONFIGURATION=-1;\nour $REF_EMAIL=\"\";\
\nour $PROGRAM=\"extract_from_pdb\";\n\n\nmy %onel\
ett_prot=&fill_onelett_prot();\nmy %threelett_prot\
=&fill_threelett_prot();\nmy %onelett_RNA=&fill_on\
elett_RNA();\nmy %threelett_RNA=&fill_threelett_RN\
A();\nmy %onelett_DNA=&fill_onelett_DNA();\nmy %th\
reelett_DNA=&fill_threelett_DNA();\n\n\n\n\n\nmy %\
onelett = (\n'P' => \\%onelett_prot,\n'D' => \\%on\
elett_DNA,\n'R' => \\%onelett_RNA\n);\n\n\nmy %thr\
eelett = (\n'P' => \\%threelett_prot,\n'D' => \\%t\
hreelett_DNA,\n'R' => \\%threelett_RNA\n);\n\n\n\n\
\n\n\n\nif($ARGV[0]=~/help/ ||$ARGV[0]=~/man/ || $\
ARGV[0]=~/HELP/ || $ARGV[0]=~/Man/ || $ARGV[0] eq \
\"-h\"  || $ARGV[0] eq \"-H\"  )\n{die \"SYNTAX: e\
xtract_from_pdb Version $VersionTag	\n	Minimum:   \
          [extract_from_pdb file] \n			   OR \n			\
     [... | extract_from_pdb]\n 	Flags (Default se\
tting on the first line)\n	   -version............\
.......[Returns the Version Number]\n           -f\
orce.....................[Forces the file to be tr\
eated like a PDB file]\n                          \
            [Regenerates the header and SEQRES fie\
lds]\n           -force_name................[Force\
s the file to be named after name]]\n           -i\
nfile.....file...........[Flag can be omited]\n			\
              [File must be pdb or fro pgm]\n     \
                                 [File can also be\
 compressed Z or gz]\n                            \
          [In the case of a compressed file, you c\
an omit the gz|Z extension]\n           -netfile..\
.................[File will be fetch from the net \
using wget]\n                                     \
 [wget or curl must be installed]\n               \
                       [ftp://ftp.gnu.org/pub/gnu/\
wget/]\n                                      [htt\
p://curl.haxx.se/]\n                              \
        [Must also be used to retrieve the file fr\
om a local pdb copy (cf netaddress)]\n           -\
netaddress................[Address used for the re\
trieving the netfile]\n                           \
           [http://www.rcsb.org/pdb/cgi/export.cgi\
/%%.pdb.gz?format=PDB&pdbId=%%&compression=gz]\n  \
                                    [http://www.ex\
pasy.ch/cgi-bin/get-pdb-entry.pl?%%]\n            \
                          [local -> will get the f\
ile from pdb_dir (see pdb_dir)]\n           -netco\
mpression............[Extension if the netfile com\
es compressed]\n                                  \
    [gz]\n           -pdb_dir...................[a\
ddress of the repertory where the pdb is installed\
]\n                                      [Supports\
 standard ftp style installation OR every stru in \
DIR]\n                                      [Give \
the ..../pdb/structure/ dir]\n                    \
                  [If value omitted, the pg gets i\
t from the env variable PDB_DIR]\n           -netc\
ompression_pg.........[gunzip]\n           -is_pdb\
_name..........name.[Returns 1 if the name is a PD\
B ID, 0 otherwise]\n           -model_type........\
...name.[Returns the model type if valid PDB name]\
\n           -is_released_pdb_name name.[Returns 1\
 if the name corresponds to a released PDB file]\n\
           -get_pdb_chains.....name...[Returns the\
 list of chains corresponding to the entry]\n     \
      -get_pdb_id.........name...[Returns the PDB \
id within the provided pdb file]\n           -get_\
fugue_name.....name...[Turns a name into a name va\
lid for fugue]\n                                  \
    [Uses the netaddress to do so]\n	   -chain....\
..FIRST..........[Extract the first chain only]\n	\
	       A B C..........[Extract Several chains if \
needed]\n		       ALL............[Extract all the \
chains]	\n           -ligand.....ALL............[E\
xtract the ligands in the chain (HETATM)]\n       \
                <name1>,<name2>[Extract All the na\
med lignds]\n	   -ligand_only...............[Extra\
ct only the ligands]\n           -ligand_list.....\
...
.<start>..<end>.[Delete from residue start to resi\
due end]\n	   -atom.......CA.............[Atoms to\
 include, ALL for all of them]\n		       CA O N...\
......[Indicate several atoms if needed]\n	   -cod\
e.......3..............[Use the 1 letter code or t\
he 3 letters code]\n	   -mode.......raw...........\
.[Output original pdb file]\n                     \
  pdb............[Output something that looks like\
 pdb]\n		       fasta..........[Output the sequenc\
es in fasta format]\n		       simple.........[Outp\
ut a format easy to parse in C ]\n            -seq\
_field..ATOM...........[Field used to extract the \
sequence]\n		       SEQRES.........[Use the comple\
te sequence]\n	   -seq.......................[Equi\
valent to  -mode fasta]\n	   -model......1........\
......[Chosen Model in an NMR file]\n           -n\
odiagnostic..............[Switches Error Messages \
off]\n           -debug.....................[Sets \
the DEBUG ON]\n           -no_remote_pdb_dir......\
...[Do not look for a remote file]\n           -ca\
che_pdb.................[Cache Value, default is $\
HOME/.t_coffee/cache, other values: NO<=> No cache\
]\n\n      Environement Variables\n           Thes\
e variables can be set from the environement\n    \
       Command line values with the corresponding \
flag superseed evironement value\n           NO_RE\
MOTE_PDB_DIR..........[Prevents the program from s\
earching remote file: faster]\n           PDB_DIR.\
...................[Indicates where PDB file must \
be fetched (localy)]\n\n	 PROBLEMS: please contact\
 cedric.notredame\\@europe.com\\n\";\n	 exit ($EXI\
T_SUCCESS);\n}\n\n$np=0;\n$n_para=$#ARGV;\n$model=\
1;\n$pdb_dir=$ENV{'PDB_DIR'};if ($pdb_dir){$pdb_di\
r.=\"/\";}\n$debug=$ENV{'DEBUG_EXTRACT_FROM_PDB'};\
\n\n$no_remote_pdb_dir=$ENV{NO_REMOTE_PDB_DIR};\n$\
HOME=$ENV{'HOME'};\nif ( $ENV{CACHE_4_TCOFFEE})\n{\
$cache=$ENV{CACHE_4_TCOFFEE};}\nelse\n{\n    $cach\
e=\"$HOME/.t_coffee/cache/\";\n}\n\n   \n$netaddre\
ss=\"http://www.rcsb.org/pdb/files/%%.pdb.gz\";\n$\
...
ARGV[$np];\n      }\n    elsif ( !($value=~/^-.*/)\
  ...
  \n    elsif ($value eq \"-nodiagnostic\"){$nodia\
...
ARGV[++$np];	\n	$is_pdb_name=1;	\n      } \n    el\
sif ($value eq \"-is_released_pdb_name\")\n      {\
\n	$pdb_file= $ARGV[++$np];	\n	$is_released_pdb_na\
...
b_chains\")\n{\n	$pdb_file= $ARGV[++$np];\n	$get_p\
db_chains=1;\n}\n    elsif ($value eq \"-get_pdb_l\
igands\")\n{\n	$get_pdb_ligands=1;\n}\n    \n    e\
lsif ($value eq \"-get_pdb_id\")\n{\n	$pdb_file= $\
ARGV[++$np];\n	$get_pdb_id=1;\n	\n}\n    \n    els\
if ( $value eq \"-get_fugue_name\")\n{\n	$pdb_file\
= $ARGV[++$np];\n	$get_fugue_name=1;\n}\n    elsif\
 ( $value eq \"-infile\")\n{\n       $pdb_file= $A\
...
alue eq \"-ligand_only\")\n{\n	$ligand_only=1;\n}\\
n}\nif ( $debug)\n{\n    print STDERR \"\\n[DEBUG:\
extract_from_pdb] NO_REMOTE_PDB_DIR: $no_remote_pd\
b_dir\\n\";\n    print STDERR \"\\n[DEBUG:extract_\
from_pdb] PDB_DIR: $pdb_dir\\n\";\n}\n\n\nif ( $is\
...
n	print \"1\";\n      }\n    else\n      {\n	print\
 \"0\";\n      }\n    exit ($EXIT_SUCCESS);\n  }\n\
if ($model_type)\n  {\n   \n    printf \"%s\", &pd\
b2model_type($pdb_file);\n    exit ($EXIT_SUCCESS)\
;\n    \n  }\n    \n\nif (!$force_name)\n{\n    $p\
db_file=~/([^\\/]*)$/;\n    $force_name=$1;\n}\n\n\
$local_pdb_file=$pdb_file;\n\nif ( $debug){print S\
TDERR \"\\n[DEBUG: extract_from_pdb] Scan For $loc\
...
db_file into $local_pdb_file and $suffix_chain \\n\
...
RESULT for $pdb_file\\n\";\n	    &myexit ($EXIT_SU\
CCESS);	\n}\n}\n\n\n\n\n%molecule_type=&pdbfile2ch\
aintype($tmp_pdb_file);\nif ( $debug){print STDERR\
 \"\\n\\tSequence Type determined\\n\";}\n\n$pdb_i\
d=&get_pdb_id ($tmp_pdb_file);\nif ( $debug){print\
...
@f_chain\\n\";\n    &myexit ($EXIT_SUCCESS);\n}\ni\
f ( $get_pdb_ligands)\n{\n    %complete_ligand_lis\
t=&get_ligand_list ($tmp_pdb_file);\n    print $co\
mplete_ligand_list{\"result\"};\n    &myexit ($EXI\
T_SUCCESS);\n}\n\nelsif ( $get_pdb_id ||$get_fugue\
_name )\n{\n    if    (@c_chain && $c_chain[0] eq \
\"FIRST\"){$pdb_id=$pdb_id.$f_chain[0];}\n    elsi\
f (@c_chain && $c_chain[0] ne \" \"){$pdb_id=$pdb_\
id.$c_chain[0];}\n    \n    print \"$pdb_id\\n\";\\
n    &myexit ($EXIT_SUCCESS);\n    \n}\nelsif ( $i\
s_pdb_name)\n{\n    printf \"1\\n\";\n    &myexit \
($EXIT_SUCCESS);\n}\n\n\n\n$structure_file=vtmpnam\
();\n\nif ( $debug){print STDERR \"\\n\\tCheck_poi\
nt #1: $tmp_pdb_file  $structure_file\\n\";}\n\n$I\
NFILE=vfopen (\"$tmp_pdb_file\", \"r\"); \n$TMP=vf\
